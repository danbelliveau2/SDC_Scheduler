...
async function checkAndUpdate() {
  ...
  try {
    ...
    // 5. Store SHA BEFORE restarting the updater itself.
    // pm2 restart sdc-scheduler-updater sends SIGTERM to this process; if storeSha()
    // ran after the restart loop it would never execute, leaving the old SHA on disk
    // and re-triggering the same download on every future restart (infinite loop).
    storeSha(remoteSha);
