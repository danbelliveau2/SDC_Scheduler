...
function alignGanttToGrid() {
  ...
  // ── Phase ribbon stripes ────────────────────────────────────────────────────
  // Draw translucent horizontal ribbons behind bars — one per phase/section group.
  // Each ribbon spans the full SVG width and covers all the rows belonging to that
  // phase. Colors come from the PHASE_BY_KEY palette (same as the grid chips).
  ...
  (function drawPhaseRibbons() {
    const svgWidth = +ganttSvg.getAttribute('width') || 2000;
    const stripeGroup = document.createElementNS(SVG_NS, 'g');
    stripeGroup.setAttribute('class', 'sdc-row-stripes');

    // Walk the rendered task rows in sort order and group consecutive rows by phase.
    const rows = [...document.querySelectorAll('#tasks-tbody tr[data-id]')];
    let runs = []; // [{ phaseKey, startIdx, endIdx }]
    rows.forEach((tr, i) => {
      const taskId = tr.dataset.id;
      const task = state.tasks.find(t => String(t.id) === String(taskId));
      const phaseKey = task?.phase || null;
      if (!phaseKey) return;
      const last = runs[runs.length - 1];
      if (last && last.phaseKey === phaseKey) { last.endIdx = i; }
      else runs.push({ phaseKey, startIdx: i, endIdx: i });
    });

    runs.forEach(run => {
      const phase = PHASE_BY_KEY[run.phaseKey];
      if (!phase || !phase.color) return;

      // Find the Gantt bar-wrapper for the first and last tasks in this run.
      const firstTr = rows[run.startIdx];
      const lastTr  = rows[run.endIdx];
      if (!firstTr || !lastTr) return;

      const firstWrap = ganttSvg.querySelector(`.bar-wrapper[data-id="${firstTr.dataset.id}"]`);
      const lastWrap  = ganttSvg.querySelector(`.bar-wrapper[data-id="${lastTr.dataset.id}"]`);
      if (!firstWrap || !lastWrap) return;

      // Get the Y extents from the bar rects inside these wrappers.
      const getBarY = (wrap) => {
        const bar = wrap.querySelector('.bar') || wrap.querySelector('rect');
        return bar ? +bar.getAttribute('y') : null;
      };
      const getBarBottom = (wrap) => {
        const bar = wrap.querySelector('.bar') || wrap.querySelector('rect');
        if (!bar) return null;
        return +bar.getAttribute('y') + +bar.getAttribute('height');
      };

      const topY    = getBarY(firstWrap);
      const bottomY = getBarBottom(lastWrap);
      if (topY == null || bottomY == null) return;

      const PAD = rowH * 0.35; // half-row padding top and bottom
      const rect = document.createElementNS(SVG_NS, 'rect');
      rect.setAttribute('x', '0');
      rect.setAttribute('y', String(topY - PAD));
      rect.setAttribute('width', String(svgWidth));
      rect.setAttribute('height', String(bottomY - topY + PAD * 2));
    ...
    if (stripeGroup.children.length > 0) {
      // Insert behind bars but above the white background
      ganttSvg.insertBefore(stripeGroup, ganttSvg.firstChild);
    }
  })();
...
/* ── Quick-jump popup ──────────────────────────────────────────────────────
   Opens a small search dropdown anchored to the "Jump to…" button.
   Filters all known projects as you type; clicking one switches to it
   (or opens it as a new tab if it isn't open yet). Ctrl+K re-opens it.
──────────────────────────────────────────────────────────────────────────── */
function showQuickJump() {
  // Toggle: clicking the button again closes the popup
  const existing = document.getElementById('quick-jump-popup');
  if (existing) { existing.remove(); return; }

  const btn = document.getElementById('btn-quick-jump');
  const r = btn.getBoundingClientRect();
  const all = uniqueValues('project');
  const openSet = new Set(state.openProjects);

  const pop = document.createElement('div');
  pop.id = 'quick-jump-popup';
  pop.className = 'quick-jump-popup';

  // Anchor right-edge to button right, flip left if it would overflow
  const W = 340;
  ...
  pop.innerHTML = `
    <div class="quick-jump-input-wrap">
      <input id="quick-jump-input" class="quick-jump-input" type="text" placeholder="Type a project number or name…" autocomplete="off" />
    </div>
    <div class="quick-jump-list" id="quick-jump-list"></div>`;
  document.body.appendChild(pop);

  const input = pop.querySelector('#quick-jump-input');
  const list  = pop.querySelector('#quick-jump-list');
  let focusIdx = -1;
  ...
  function renderRows(filter) {
    ...
    if (matches.length === 0) {
      list.innerHTML = `<div class="quick-jump-empty">${q ? 'No project matches "' + escapeHtml(q) + '"' : 'No projects yet.'}</div>`;
      focusIdx = -1;
      return;
    }
    ...
    list.innerHTML = matches.map((p, i) => {
      ...
      return `<button class="quick-jump-row" data-project="${escapeHtml(p)}" data-idx="${i}" type="button">
        <span class="quick-jump-row-label">${escapeHtml(p)}</span>
        ${isOpen ? '<span class="quick-jump-row-badge">open</span>' : ''}
      </button>`;
    }).join('');
    focusIdx = -1;

    list.querySelectorAll('.quick-jump-row').forEach(row => {
      row.addEventListener('click', () => jumpTo(row.dataset.project));
    });
  }
  ...
  function jumpTo(project) {
    ...
    pop.remove();
    render();
    // Switch to Schedule view so the tab is visible
    document.querySelector('.tab[data-view="schedule"]')?.click();
  }

  function moveFocus(dir) {
    const rows = [...list.querySelectorAll('.quick-jump-row')];
    if (!rows.length) return;
    rows.forEach(r => r.classList.remove('is-focused'));
    focusIdx = Math.max(0, Math.min(rows.length - 1, focusIdx + dir));
    rows[focusIdx]?.classList.add('is-focused');
  ...
  input.addEventListener('keydown', (e) => {
    if (e.key === 'ArrowDown') { e.preventDefault(); moveFocus(1); }
      ...
      const focused = list.querySelector('.quick-jump-row.is-focused');
      if (focused) { jumpTo(focused.dataset.project); return; }
      // If only one result, jump to it directly
      const rows = list.querySelectorAll('.quick-jump-row');
      if (rows.length === 1) jumpTo(rows[0].dataset.project);
    }
...
async function init() {
  ...
  document.getElementById('btn-quick-jump').addEventListener('click', showQuickJump);

  // Ctrl+K global shortcut to open the quick-jump popup from anywhere
  document.addEventListener('keydown', (e) => {
