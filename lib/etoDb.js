...
// Formats in LOCAL time, deliberately — not toISOString().slice(0, 10).
//
// ETO stores these as SQL Server datetimes carrying a local business date, and
// the driver hands them back as local-midnight-plus-offset (a receipt logged on
// 2026-06-02 arrives as Jun 02 20:00 EDT). toISOString() converts to UTC first,
// which rolls any evening timestamp to the NEXT day: PO 104589 was received 6/2
// and displayed as 2026-06-03. Measured 2026-08-26 on the Parts in Shop badge,
// but it applies to every date this helper produces — PO date, ETA, delivery —
// so the vendor PO tracker has had the same off-by-one on evening receipts.
//
// Reading the local Y/M/D components keeps the date ETO actually recorded.
// Existing rows self-correct: the 30-min sync rewrites these columns each pass.
function isoDate(d) {
  if (!d) return null;
  const dt = new Date(d);
