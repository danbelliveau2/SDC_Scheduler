    ...
    if (!hits || hits.length === 0) {
      ...
      await mysqlPool.query(
        `UPDATE shop_parts SET eto_received_qty = NULL, eto_po_qty = NULL,
           eto_received_on = NULL, eto_synced_at = NOW() WHERE id = ?`, [r.id]
      );
      continue;
    }
    out.matched++;
    ...
    if (fully && fresh && !r.part_complete) {
      ...
      await mysqlPool.query(
        `UPDATE shop_parts SET eto_received_qty = ?, eto_po_qty = ?, eto_received_on = ?,
           eto_synced_at = NOW(), part_complete = 1, completed_on = ?, completed_source = ?
         WHERE id = ?`,
      ...
      out.detail.push({ id: r.id, part_no: r.part_no, po: r.eto_po,
        result: r.part_complete ? 'already complete' : (fully && !fresh ? 'HELD: receipt predates row' : 'open'),
        received: `${rcvdQty}/${target}`, on: rcvdOn });
      if (dryRun) continue;
      await mysqlPool.query(
        `UPDATE shop_parts SET eto_received_qty = ?, eto_po_qty = ?, eto_received_on = ?,
           eto_synced_at = NOW() WHERE id = ?`,
        [rcvdQty, poQty, rcvdOn, r.id]
      );
    }
  }
  return out;
