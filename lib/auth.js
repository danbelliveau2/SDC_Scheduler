...
function signToken(user) {
  return jwt.sign(
    // token_version: pinned at sign-in so the active-flag middleware in
    // server.js can tell a since-revoked token from a current one (see
    // db.js's users.token_version column). `|| 0` covers a `user` row
    // fetched before that column existed on this connection's schema cache.
    { id: user.id, email: user.email, name: user.name, role: user.role, avatar_color: user.avatar_color, token_version: user.token_version || 0 },
    JWT_SECRET,
...
// '/api/auth/sso' is public for the same reason as login: it IS the
// authentication step. It authenticates by verifying a signature from the ETC
// Planner rather than a password.
// '/api/auth/revoke-session' is public for a related but different reason:
// it's called SERVER-TO-SERVER by the ETC Planner (from ITS OWN logout
// action), not by a signed-in browser, so there is no per-user JWT to check
// here — it verifies the shared SCHEDULER_SHARED_TOKEN bearer itself,
// exactly like /api/integration/* does on the Planner's side.
const PUBLIC_PATHS = new Set(['/health', '/api/auth/login', '/api/auth/register', '/api/auth/sso', '/api/auth/revoke-session']);

function requireAuth(req, res, next) {
  if (PUBLIC_PATHS.has(req.path)) return next();
  // Customer share links: the share middleware in server.js already
  // validated the token, scoped the request to one project, and stamped a
  // read-only viewer identity. Nothing further to check here.
  if (req.shareProject) return next();
