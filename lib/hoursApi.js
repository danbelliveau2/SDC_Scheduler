...
async function _fetchJobHours(ids, key) {
  // One request per job, in parallel. A single failure fails the whole read
  // rather than silently returning a partial total — an hours page that quietly
  // drops one job of a "1145 / 1146" pair is worse than one that says it failed.
  const perJob = await Promise.all(ids.map(id => _fetchOneJob(id)));
  const data = _summarise(perJob.flat(), ids);
  _cacheSet(key, data);
  return data;
}
