...
function start({ pool, emailSvc, etoDb, io, ops, hoursApi }) {
  ...
  // Nightly customer-snapshot refresh at 03:00 ET — regenerates every project's
  // static snapshot file (lib/snapshotRender.js) so a link handed to a client
  // stays current without anyone remembering to click "Refresh snapshot".
  // On-demand refresh (routes/projects.js) still works independently of this.
  {
    const snapshotRender = require('./snapshotRender');
    const snapshotTick = async () => {
      try {
        const [rows] = await pool.query('SELECT id, name, snapshot_token FROM projects WHERE snapshot_token IS NOT NULL');
        let ok = 0;
        for (const row of rows) {
          try {
            await snapshotRender.generateSnapshot(pool, { token: row.snapshot_token, projectName: row.name });
            await pool.query('UPDATE projects SET snapshot_updated_at = NOW() WHERE id = ?', [row.id]);
            ok++;
          } catch (e) {
            console.warn(`[cron] snapshot refresh failed for project ${row.name}:`, e.message);
          }
        }
        if (rows.length) console.log(`[cron] customer snapshot refresh: ${ok}/${rows.length} regenerated`);
      } catch (e) {
        console.warn('[cron] customer snapshot sweep failed:', e.message);
      }
    };
    if (cron) {
      cron.schedule('0 3 * * *', snapshotTick, { timezone: 'America/New_York' });
      console.log('[cron] Registered: nightly customer snapshot refresh at 03:00 ET');
    } else {
      let lastSnapshotDay = '';
      setInterval(() => {
        const now = new Date();
        if (now.getHours() === 3 && now.getMinutes() < 30) {
          const tag = now.toISOString().slice(0, 10);
          if (lastSnapshotDay !== tag) { lastSnapshotDay = tag; snapshotTick(); }
        }
      }, 20 * 60 * 1000);
      console.log('[cron] node-cron not installed — nightly snapshot refresh via 20m setInterval fallback');
    }
  }
